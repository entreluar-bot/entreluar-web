...
export default async function Blog() {
  ...
  const categories = ["Confissões de Madrugada", "Sobrevivendo com Humor", "Estudei para te explicar", "Drops do Insta", "Geral"];
  ...
  journal?.forEach(post => {
    ...
    if (groupedJournal[cat]) {
      ...
      groupedJournal["Geral"].push(post);
    }
  });

  const activeCategories = categories.filter(c => groupedJournal[c].length > 0);
