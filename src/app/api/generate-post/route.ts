...
export async function POST(req: Request) { 
  try { 
    ...
    const prompt = `Você é a Luana, criadora da Entreluar Beauty. Escreva um artigo completo para a categoria "${category || "Diário"}" do seu blog. Tema: "${title || "Crônica de uma mulher madura"}". NOTAS PESSOAIS DA LUANA (fatos que aconteceram): "${impressions || "Dê sua visão pessoal."}" ATENÇÃO: NÃO copie as notas pessoais exatamente como foram escritas! A Luana apenas jogou ali alguns fatos soltos. O seu dever é DIGERIR esses fatos e reescrevê-los de forma super envolvente, com humor e profundidade, como se você estivesse contando isso para uma amiga num áudio. Use tags HTML ESTRITAS (<p>, <h3> com emojis, <i>, <strong>, <ul>, <ol>). Sem crases no início e no fim. Escreva em PRIMEIRA PESSOA. Seja vulnerável, faça graça das tragédias da idade e foque no autocuidado. Retorne ESTRITAMENTE UM JSON VÁLIDO sem formatação markdown: { "text": "texto HTML", "imagePrompt": "A highly detailed, realistic cinematic photograph of [descrição em inglês de uma cena acolhedora, madura e estética (sem pessoas mostrando o rosto de perto) que ilustre a vibe deste texto]" }`; 
    const contents = []; 
